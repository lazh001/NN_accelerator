...
class AXIMaster(params: AXIParams) extends AXIBase(params) {
  ...
  def setup() = {
    aw.bits.id := 0.U
    aw.bits.addr := 0.U
    aw.bits.len := (params.lenBits - 1).U
    aw.bits.size := 2.U
    aw.bits.burst := 1.U
    aw.bits.lock := 0.U
    aw.bits.cache := 2.U
    ...
    ar.bits.id := 0.U
    ar.bits.addr := 0.U
    ar.bits.len := (params.lenBits - 1).U
    ar.bits.size := 2.U
    ar.bits.burst := 1.U
    ar.bits.lock := 0.U
    ar.bits.cache := 2.U
...
class XilinxAXIMaster(params: AXIParams) extends AXIBase(params) {
  ...
  def setup() = {
    ...
    WDATA := 0.U
    WSTRB := Fill(params.strbBits, true.B)
    WLAST := false.B
    WUSER := 0.U
    WVALID := false.B
